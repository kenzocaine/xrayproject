...
def load_mask_names(path = ''):
    # This function obtains the list of image-IDs for images that have masks:
    list_of_mask_filenames = get_filenames(path)
    masked_IDs = list(set([ int(os.path.basename(masked_img).split('_')[1]) \
        for masked_img in list_of_mask_filenames]))
    return masked_IDs

def load_masked_img_names(path = ''):
    # This function obtains the list of image-filepaths which have masks:
    list_of_img_filenames = get_filenames(path)
    list_of_masked_img_numbers = load_mask_names(path)
    return masked_IDs
